...
int main(int argc, char *argv[])
{
    ...
    Range rangeR(15, 15, 1);

    pupil = new Pupil(56, 70, 15);
...
//        rangeY = Range(pupil->y-5, pupil->y+5, 1);
        ...
        distance = exp(-5*((pupil->x - e.x)*(pupil->x - e.x) + (pupil->y - e.y)*(pupil->y - e.y))/((pupil->r)*(pupil->r)));

        for (double x = rangeX.begin(); x <= rangeX.end(); x = rangeX.iter()) {
            Xs = (x-e.x)*(x-e.x);
            for (double y = rangeY.begin(); y <= rangeY.end(); y = rangeY.iter()) {
                Ys = (y-e.y)*(y-e.y);
                for (double r = rangeR.begin(); r <= rangeR.end(); r = rangeR.iter()) {
                    Rs = r*r;
                    delta = (Xs + Ys) / Rs;
                    ...
                    if (delta <= 1) {
                        pos = rangeX.pos() + rangeX.size() * (rangeY.pos() + rangeY.size() * rangeR.pos());

                        H[pos] = H[pos] * pExp->val(e.t - H_times[pos]) + distance/Rs;
                        H_times[pos] = e.t;
                        change = true;
                    }
