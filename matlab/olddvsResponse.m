...
dh = hist(nonzeros(distances(:)), 0:0.001:max(max(distances)));

std(nonzeros(distances(:)))
median(nonzeros(distances(:)))
mean(nonzeros(distances(:)))
...
tic
%dvsMonteCarlo( deltas, noiseratio, distanceThreshold, iterations )
[input_output_count cellClasses] = dvsMonteCarlo(deltas, noiseratio, distanceThreshold, iterations);
toc
